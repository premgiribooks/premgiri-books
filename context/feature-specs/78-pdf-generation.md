...
## ⚠️ v3 Supersession Note — Puppeteer → @react-pdf/renderer

**This spec must NOT be implemented before v3 spec 103 (PDF Engine Migration) is
confirmed out of scope for the current milestone.**

This spec selects **Puppeteer** as the PDF renderer (see the Goal section's library
decision). However, v3 spec 103 (`context-v3/feature-specs/103-pdf-engine-migration.md`)
**removes Puppeteer entirely** from the application and replaces it with
`@react-pdf/renderer`. The two specs are directly incompatible:

| | This spec (78, Puppeteer) | v3 spec 103 (@react-pdf/renderer) |
|---|---|---|
| Library | `puppeteer` | `@react-pdf/renderer` |
| Renderer | HTML + CSS → Chromium → PDF | React component tree → PDF |
| Electron bundle size | +Chromium (~200MB) | No bundled browser |
| Template format | HTML strings + `print.css` | JSX React components |

### If implementing this spec BEFORE v3 begins:

Implement using Puppeteer exactly as specified. The implementation will be replaced when
v3 spec 103 is implemented.

### If implementing this spec AFTER v3 spec 103 is merged:

**Do not use Puppeteer.** Replace the `renderHtmlToPdf` core with a `renderPdfBuffer`
function backed by `@react-pdf/renderer`, and replace all `build*Html` template
functions with React component trees (e.g. `<SalesInvoicePdf />`, `<TrialBalancePdf />`).
The `ReportExportTable[]` contract from spec 77 is still reused — only the rendering
step changes.

The `src/lib/pdf-generation.ts` file's public interface (`renderHtmlToPdf` function
signature and `PdfOptions` type) should be designed so it can be replaced with a new
implementation without changing any callers.

### v4 compatibility

In v4, document PDF generation moves into the individual microservice that owns the
document (e.g. Sales Invoice PDF is generated by the Sales microservice). The shared
`renderHtmlToPdf` / `renderPdfBuffer` core is kept as a shared library in a v4
`@premgiri/pdf` internal package. No architectural change is needed to this spec's
design to support that — the module boundaries here already match the v4 service
boundaries.
