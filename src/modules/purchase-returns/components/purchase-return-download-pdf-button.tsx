...
export function PurchaseReturnDownloadPdfButton({ purchaseReturnId }: PurchaseReturnDownloadPdfButtonProps) {
  ...
  async function handleDownload() {
    setIsPreparing(true);
    try {
      const { usedPrintFallback } = await downloadOrPrintDocument(`/purchase/returns/${purchaseReturnId}/pdf`);
      if (usedPrintFallback) {
        toast.info('Choose "Save as PDF" in the print dialog to download this purchase return.');
      }
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Failed to prepare the purchase return PDF.");
    } finally {
