...
function gstRateLabel(rate: DebitNoteGstRateOption): string {
  return rate.cessPercent > 0 ? `${rate.name} (${rate.ratePercent}% + ${rate.cessPercent}% cess)` : `${rate.name} (${rate.ratePercent}%)`;
}

interface DebitNoteLineEditorProps {
  gstRates: DebitNoteGstRateOption[];
}

/** Freeform adjustment lines (41-debit-note.md's Data Model, mirroring
 * 40-credit-note.md's identical concept) — deliberately not derived from a
 * product or invoice line, so this is a plain add/remove row editor. */
export function DebitNoteLineEditor({ gstRates }: DebitNoteLineEditorProps) {
                ...
                <TableCell className="min-w-56">
                  <FormField
                    ...
                    render={({ field: descriptionField }) => (
                      <FormItem>
                        <FormControl>
                          <Input {...descriptionField} placeholder="e.g. Additional freight charge" />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
