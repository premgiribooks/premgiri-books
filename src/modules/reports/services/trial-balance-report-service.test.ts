...
describe("trialBalanceReportService.getTrialBalanceReport", () => {
  it("gates on the reports:view permission", async () => {
    await trialBalanceReportService.getTrialBalanceReport({ financialYearId: FY_ID, asOfDate: "2026-04-15" });
  ...
  it("throws when the financial year does not belong to the caller's company", async () => {
    ...
    await expect(
      trialBalanceReportService.getTrialBalanceReport({ financialYearId: FY_ID, asOfDate: "2026-04-15" })
  ...
  it("rejects an as-of date before the financial year's startDate", async () => {
    await expect(
      trialBalanceReportService.getTrialBalanceReport({ financialYearId: FY_ID, asOfDate: "2026-03-31" })
  ...
  it("rejects an as-of date after the financial year's endDate", async () => {
    await expect(
      trialBalanceReportService.getTrialBalanceReport({ financialYearId: FY_ID, asOfDate: "2027-04-01" })
    ).rejects.toThrow("As-of date must fall within the selected financial year's date range.");
  });

  it("accepts an as-of date on either boundary of the financial year's range", async () => {
    await expect(
      trialBalanceReportService.getTrialBalanceReport({ financialYearId: FY_ID, asOfDate: "2026-04-01" })
    ).resolves.toBeDefined();
    await expect(
      trialBalanceReportService.getTrialBalanceReport({ financialYearId: FY_ID, asOfDate: "2027-03-31" })
    ).resolves.toBeDefined();
  });
  ...
  it("never re-derives the grand total — it is copied from voucherEngine.getTrialBalance's own result", async () => {
    ...
    const report = await trialBalanceReportService.getTrialBalanceReport({
