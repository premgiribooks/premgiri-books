...
interface CustomerStatementTableProps {
  report: CustomerStatementReport;
}
...
export function CustomerStatementTable({ report }: CustomerStatementTableProps) {
  return (
    <Table>
      <TableHeader>
        <TableRow>
        ...
        {report.lines.length === 0 ? (
          <TableRow>
            <TableCell colSpan={5} className="py-8 text-center text-sm text-muted-foreground">
              No voucher entries found for {report.customerName} in the selected range.
            </TableCell>
          </TableRow>
        ) : (
