...
export function GoodsReceiptNoteDownloadPdfButton({ goodsReceiptNoteId }: GoodsReceiptNoteDownloadPdfButtonProps) {
  ...
  async function handleDownload() {
    setIsPreparing(true);
    try {
      const { usedPrintFallback } = await downloadOrPrintDocument(`/purchase/receipts/${goodsReceiptNoteId}/pdf`);
      if (usedPrintFallback) {
        toast.info('Choose "Save as PDF" in the print dialog to download this goods receipt note.');
      }
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Failed to prepare the goods receipt note PDF.");
    } finally {
