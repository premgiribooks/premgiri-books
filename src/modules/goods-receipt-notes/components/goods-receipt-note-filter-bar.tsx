...
import { GOODS_RECEIPT_NOTE_STATUS_LABELS } from "@/modules/goods-receipt-notes/components/goods-receipt-note-status-badge";
import { GOODS_RECEIPT_NOTE_STATUS_VALUES } from "@/modules/goods-receipt-notes/validation/goods-receipt-note-schema";
import type { GoodsReceiptNoteSupplierOption } from "@/types/goods-receipt-note";
...
interface GoodsReceiptNoteFilterBarProps {
  suppliers: GoodsReceiptNoteSupplierOption[];
}

/**
 * Search + status/supplier filters for the goods receipt note list — mirrors
 * delivery-challan-filter-bar.tsx's URL-state pattern exactly.
 */
export function GoodsReceiptNoteFilterBar({ suppliers }: GoodsReceiptNoteFilterBarProps) {
  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
      ...
      <FilterSelect
        ...
        options={GOODS_RECEIPT_NOTE_STATUS_VALUES.map((value) => ({
          value,
          label: GOODS_RECEIPT_NOTE_STATUS_LABELS[value],
        }))}
      />
