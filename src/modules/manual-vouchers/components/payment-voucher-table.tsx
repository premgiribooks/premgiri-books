...
/** "Paid To" summary — every Debit entry's ledger, comma-joined; a single-Debit voucher (the common case) shows just that one name. */
function paidToSummary(voucher: PostedVoucher, ledgerNameById: ReadonlyMap<string, string>): string {
  const debitLedgerIds = voucher.entries.filter((entry) => entry.entryType === "DEBIT").map((entry) => entry.ledgerId);
      ...
      <TableBody>
        {vouchers.map((voucher) => (
          <TableRow key={voucher.id}>
            ...
            <TableCell className="text-muted-foreground">{paidToSummary(voucher, ledgerNameById)}</TableCell>
            ...
            <TableCell className="text-right">
              <Button
                ...
                nativeButton={false}
                render={
                  <Link href={`/accounting/payment-vouchers/${voucher.id}`} aria-label="View payment voucher">
                    <Eye size={16} />
                  </Link>
                }
              />
