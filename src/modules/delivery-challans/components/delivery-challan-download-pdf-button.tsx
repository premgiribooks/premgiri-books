...
export function DeliveryChallanDownloadPdfButton({ deliveryChallanId }: DeliveryChallanDownloadPdfButtonProps) {
  ...
  async function handleDownload() {
    setIsPreparing(true);
    try {
      const { usedPrintFallback } = await downloadOrPrintDocument(`/sales/challans/${deliveryChallanId}/pdf`);
      if (usedPrintFallback) {
        toast.info('Choose "Save as PDF" in the print dialog to download this delivery challan.');
      }
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Failed to prepare the delivery challan PDF.");
    } finally {
