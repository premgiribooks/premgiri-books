...
// extended with the effective-tax-total and round-off math this document's
// tax-override feature needs (gstEngine.calculateDocument has no concept of
// a per-line override, so the header's tax totals cannot be taken from it
// directly once any line is overridden). No I/O, no engine calls. Every sum
// runs in integer paise to avoid float drift.
 ...
 * the GST Engine's calculateDocument — mirrors sales-order-calculations.ts's
 ...
 * sales-invoice-service.ts's buildLine.
 */
...
export interface RoundOffResult {
  ...
  grandTotal: number;
  /** `grandTotal - exactTotal` — positive when rounded UP (posts CREDIT),
   * negative when rounded DOWN (posts DEBIT), zero when no rounding entry is
   * needed (38-sales-invoice.md's Ledger Posting rule). */
  roundOff: number;
}
