...
export function CreditNoteDownloadPdfButton({ creditNoteId }: CreditNoteDownloadPdfButtonProps) {
  ...
  async function handleDownload() {
    setIsPreparing(true);
    try {
      const { usedPrintFallback } = await downloadOrPrintDocument(`/sales/credit-notes/${creditNoteId}/pdf`);
      if (usedPrintFallback) {
        toast.info('Choose "Save as PDF" in the print dialog to download this credit note.');
      }
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Failed to prepare the credit note PDF.");
    } finally {
