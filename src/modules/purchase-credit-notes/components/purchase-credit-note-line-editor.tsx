...
import type { CreatePurchaseCreditNoteInput } from "@/modules/purchase-credit-notes/validation/purchase-credit-note-schema";
import type { PurchaseCreditNoteGstRateOption } from "@/types/purchase-credit-note";
...
function gstRateLabel(rate: PurchaseCreditNoteGstRateOption): string {
  return rate.cessPercent > 0 ? `${rate.name} (${rate.ratePercent}% + ${rate.cessPercent}% cess)` : `${rate.name} (${rate.ratePercent}%)`;
}

interface PurchaseCreditNoteLineEditorProps {
  gstRates: PurchaseCreditNoteGstRateOption[];
}

/** Freeform adjustment lines (mirrors credit-note-line-editor.tsx) —
 * deliberately not derived from a product or invoice line, so this is a
 * plain add/remove row editor rather than a capped, invoice-driven one like
 * purchase-return-form.tsx's. */
export function PurchaseCreditNoteLineEditor({ gstRates }: PurchaseCreditNoteLineEditorProps) {
  const { control, setValue } = useFormContext<CreatePurchaseCreditNoteInput>();
