...
export function SalesReturnDownloadPdfButton({ salesReturnId }: SalesReturnDownloadPdfButtonProps) {
  ...
  async function handleDownload() {
    setIsPreparing(true);
    try {
      const { usedPrintFallback } = await downloadOrPrintDocument(`/sales/returns/${salesReturnId}/pdf`);
      if (usedPrintFallback) {
        toast.info('Choose "Save as PDF" in the print dialog to download this sales return.');
      }
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Failed to prepare the sales return PDF.");
    } finally {
