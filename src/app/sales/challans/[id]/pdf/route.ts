...
function downloadFilename(challanNumber: string): string {
  return `${challanNumber.replace(/[^a-zA-Z0-9._-]/g, "_")}.pdf`;
}

/**
 * Delivers a Delivery Challan as a downloadable PDF — a thin Route Handler
 * with no business logic of its own (code-standards.md's Business Logic
 * rule): `deliveryChallanService.getDeliveryChallan` re-checks its own
 * `sales`/`view` permission and company scoping exactly as the detail page
 * does, `buildDeliveryChallanHtml` renders the already-loaded document, and
...
export async function GET(_request: Request, { params }: RouteParams): Promise<NextResponse> {
  ...
  try {
    ...
    const html = buildDeliveryChallanHtml(deliveryChallan);
    ...
    return new NextResponse(new Uint8Array(result.buffer), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${downloadFilename(deliveryChallan.challanNumber)}"`,
      },
    });
  } catch (error) {
    ...
    logger.error({ err: error }, "Unhandled error generating delivery challan PDF");
