...
import { PaymentVoucherCancelButton } from "@/modules/manual-vouchers/components/payment-voucher-cancel-button";
...
/**
 * Read-only detail view (52-payment-voucher.md's UI section) — no Edit, per
 * every voucher in this project being immutable once posted; correcting one
 * means Cancel + re-entry.
 */
export default async function PaymentVoucherDetailPage({ params }: PaymentVoucherDetailPageProps) {
  ...
  const [voucher, ledgerOptions, isAdmin, canCancel] = await Promise.all([
    paymentVoucherService.getPaymentVoucher(id),
      ...
      <div className="flex flex-col gap-6 p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          {canCancel && voucher.status === "POSTED" ? (
            <PaymentVoucherCancelButton id={voucher.id} voucherNumber={voucher.voucherNumber} />
          ) : null}
        </div>
