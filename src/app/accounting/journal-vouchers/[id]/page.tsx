...
import { JournalVoucherCancelButton } from "@/modules/manual-vouchers/components/journal-voucher-cancel-button";
...
/**
 * Read-only detail view (55-journal-voucher.md's UI section) — no Edit, per
 * every voucher in this project being immutable once posted; correcting one
 * means Cancel + re-entry. Cancel is gated on `approve`, same as Post (see
 * `journalVoucherService.cancelJournalVoucher`).
 */
export default async function JournalVoucherDetailPage({ params }: JournalVoucherDetailPageProps) {
  ...
  const [voucher, ledgerOptions, isAdmin, canCancel] = await Promise.all([
    journalVoucherService.getJournalVoucher(id),
      ...
      <div className="flex flex-col gap-6 p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          {canCancel && voucher.status === "POSTED" ? (
            <JournalVoucherCancelButton id={voucher.id} voucherNumber={voucher.voucherNumber} />
          ) : null}
        </div>
