...
import { ContraVoucherCancelButton } from "@/modules/manual-vouchers/components/contra-voucher-cancel-button";
...
interface ContraVoucherDetailPageProps {
...
/**
 * Read-only detail view (54-contra-voucher.md's UI section) — no Edit, per
 * every voucher in this project being immutable once posted; correcting one
 * means Cancel + re-entry.
 */
export default async function ContraVoucherDetailPage({ params }: ContraVoucherDetailPageProps) {
  ...
  const [voucher, ledgerOptions, isAdmin, canCancel] = await Promise.all([
    contraVoucherService.getContraVoucher(id),
      ...
      <div className="flex flex-col gap-6 p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          {canCancel && voucher.status === "POSTED" ? (
            <ContraVoucherCancelButton id={voucher.id} voucherNumber={voucher.voucherNumber} />
          ) : null}
        </div>
