...
describe("GET /reports/suppliers/purchase-summary/export", () => {
  ...
  describe("format=pdf", () => {
    it("returns a %PDF--prefixed buffer with the correct headers on success", async () => {
      ...
      expect(response.headers.get("Content-Disposition")).toBe(
        'attachment; filename="Supplier-Purchase-Summary-2027-04-01_to_2027-04-30.pdf"'
      );
      const buffer = Buffer.from(await response.arrayBuffer());
      expect(buffer.subarray(0, 5).toString("ascii")).toBe("%PDF-");
    });

    it("reuses the exact same toSupplierPurchaseSummaryExportTable shaping as the xlsx path — one shaping function, two rendering targets", async () => {
      const sharedTables = [{ sheetName: "Supplier Purchase Summary", columns: [], rows: [] }];
      toSupplierPurchaseSummaryExportTableMock.mockReturnValue(sharedTables);
