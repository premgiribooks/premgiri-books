...
interface SupplierOutstandingPageProps {
...
export default async function SupplierOutstandingPage({ searchParams }: SupplierOutstandingPageProps) {
      ...
      <AppShell isAdmin={isAdmin}>
        <div className="flex flex-col gap-6 p-6">
          ...
          <div className="flex flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-border py-16 text-center">
            <p className="text-sm text-muted-foreground">Create a financial year before viewing the Supplier Outstanding Report.</p>
          </div>
        </div>
      </AppShell>
    );
  ...
  let report: SupplierOutstandingReport | null = null;
  let errorMessage: string | null = null;
  try {
    report = await supplierReportService.getSupplierOutstandingReport({
    ...
    <AppShell isAdmin={isAdmin}>
      <div className="flex flex-col gap-6 p-6">
        ...
        {errorMessage ? (
          ...
          <SupplierOutstandingTable report={report!} />
        )}
      </div>
    </AppShell>
  );
