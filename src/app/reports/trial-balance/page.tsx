...
import { TrialBalanceGroupTree } from "@/modules/reports/components/trial-balance-group-tree";
...
export default async function TrialBalancePage({ searchParams }: TrialBalancePageProps) {
      ...
      <AppShell isAdmin={isAdmin}>
        <div className="flex flex-col gap-6 p-6">
          ...
          <div className="flex flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-border py-16 text-center">
            <p className="text-sm text-muted-foreground">Create a financial year before viewing the Trial Balance.</p>
          </div>
        </div>
      </AppShell>
    );
  ...
  let report: TrialBalanceReport | null = null;
  let errorMessage: string | null = null;
  try {
    report = await trialBalanceReportService.getTrialBalanceReport({
    ...
    <AppShell isAdmin={isAdmin}>
      <div className="flex flex-col gap-6 p-6">
        ...
        {errorMessage ? (
          ...
          <TrialBalanceGroupTree report={report!} />
        )}
      </div>
    </AppShell>
  );
