...
const {
  ...
  getCustomerStatementMock,
  exportToExcelBufferMock,
  toCustomerStatementExportTableMock,
  ...
  getCustomerStatementMock: vi.fn(),
  exportToExcelBufferMock: vi.fn(),
  toCustomerStatementExportTableMock: vi.fn(),
...
vi.mock("@/modules/reports/customers/services/customer-report-service", () => ({
  customerReportService: { getCustomerStatement: getCustomerStatementMock },
}));
vi.mock("@/lib/excel-export", () => ({
  exportToExcelBuffer: exportToExcelBufferMock,
}));
vi.mock("@/engines/reporting/customer-reports", () => ({
  toCustomerStatementExportTable: toCustomerStatementExportTableMock,
}));
...
function request(query: string): Request {
  return new Request(`http://localhost/reports/customers/statement/export${query}`);
}

const VALID_QUERY = "?customerId=11111111-1111-4111-8111-111111111111&dateFrom=2027-01-01&dateTo=2027-03-31";

describe("GET /reports/customers/statement/export", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    getCurrentCompanyUserMock.mockResolvedValue({ id: "user-1", companyId: "company-1", role: "Accountant" });
    assertPermissionMock.mockResolvedValue(undefined);
    getCustomerStatementMock.mockResolvedValue({
      customerId: "11111111-1111-4111-8111-111111111111",
      customerName: "Acme Retail",
      openingBalance: 1000,
      lines: [],
      closingBalance: 1000,
    } as CustomerStatementReport);
    toCustomerStatementExportTableMock.mockReturnValue([
      { sheetName: "Customer Statement", title: "Acme Retail", columns: [], rows: [] },
    ]);
  ...
  it("returns 400 for a missing/invalid customerId or date range — never reaching the service", async () => {
    const response = await GET(request("?customerId=not-a-uuid&dateFrom=2027-01-01&dateTo=2027-03-31"));
  ...
  it("returns 400 when dateTo is before dateFrom", async () => {
    const response = await GET(
      request("?customerId=11111111-1111-4111-8111-111111111111&dateFrom=2027-03-31&dateTo=2027-01-01")
    );
  ...
  it("maps AuthorizationError (missing reports:export) to 403, before the service is ever called", async () => {
    ...
    expect(getCustomerStatementMock).not.toHaveBeenCalled();
  });

  it("maps a plain AppError (e.g. customer not found) to 400", async () => {
    getCustomerStatementMock.mockRejectedValue(new AppError("Customer not found."));
  ...
  it("returns the workbook buffer with the correct headers on success", async () => {
    ...
    expect(response.headers.get("Content-Disposition")).toBe(
      'attachment; filename="Customer-Statement-Acme_Retail-2027-01-01_to_2027-03-31.xlsx"'
    );
  ...
  describe("format=pdf", () => {
    it("returns a %PDF--prefixed buffer with the correct headers on success", async () => {
      ...
      expect(response.headers.get("Content-Disposition")).toBe(
        'attachment; filename="Customer-Statement-Acme_Retail-2027-01-01_to_2027-03-31.pdf"'
      );
      const buffer = Buffer.from(await response.arrayBuffer());
      expect(buffer.subarray(0, 5).toString("ascii")).toBe("%PDF-");
    });

    it("reuses the exact same toCustomerStatementExportTable shaping as the xlsx path — one shaping function, two rendering targets", async () => {
      const sharedTables = [{ sheetName: "Customer Statement", title: "Acme Retail", columns: [], rows: [] }];
      toCustomerStatementExportTableMock.mockReturnValue(sharedTables);
    ...
    it("applies the same filter validation as the xlsx path", async () => {
      const response = await GET(request("?customerId=not-a-uuid&dateFrom=2027-01-01&dateTo=2027-03-31&format=pdf"));
