...
const {
  ...
  toCustomerOutstandingExportTableMock,
  ...
  toCustomerOutstandingExportTableMock: vi.fn(),
...
vi.mock("@/modules/reports/customers/services/customer-report-service", () => ({
  customerReportService: { getCustomerOutstandingReport: getCustomerOutstandingReportMock },
}));
vi.mock("@/lib/excel-export", () => ({
  exportToExcelBuffer: exportToExcelBufferMock,
}));
vi.mock("@/engines/reporting/customer-reports", () => ({
  toCustomerOutstandingExportTable: toCustomerOutstandingExportTableMock,
}));
...
function request(query: string): Request {
  return new Request(`http://localhost/reports/customers/outstanding/export${query}`);
}

const VALID_QUERY = "?financialYearId=11111111-1111-4111-8111-111111111111&asOfDate=2027-03-31";

describe("GET /reports/customers/outstanding/export", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    getCurrentCompanyUserMock.mockResolvedValue({ id: "user-1", companyId: "company-1", role: "Accountant" });
    assertPermissionMock.mockResolvedValue(undefined);
    getCustomerOutstandingReportMock.mockResolvedValue({ rows: [] } as CustomerOutstandingReport);
    toCustomerOutstandingExportTableMock.mockReturnValue([{ sheetName: "Customer Outstanding", columns: [], rows: [] }]);
  ...
  it("maps AuthorizationError (missing reports:export) to 403, before the service is ever called", async () => {
    ...
    expect(getCustomerOutstandingReportMock).not.toHaveBeenCalled();
  });

  it("maps a plain AppError (e.g. an invalid financial year) to 400", async () => {
    getCustomerOutstandingReportMock.mockRejectedValue(new AppError("Financial year not found."));
  ...
  it("passes an explicit status filter through to the service", async () => {
    await GET(request(`${VALID_QUERY}&status=inactive`));

    expect(getCustomerOutstandingReportMock).toHaveBeenCalledWith(
  ...
  it("returns the workbook buffer with the correct headers on success", async () => {
    ...
    expect(response.headers.get("Content-Disposition")).toBe(
      'attachment; filename="Customer-Outstanding-2027-03-31.xlsx"'
    );
  ...
  describe("format=pdf", () => {
    it("returns a %PDF--prefixed buffer with the correct headers on success", async () => {
      ...
      expect(response.headers.get("Content-Disposition")).toBe(
        'attachment; filename="Customer-Outstanding-2027-03-31.pdf"'
      );
      const buffer = Buffer.from(await response.arrayBuffer());
      expect(buffer.subarray(0, 5).toString("ascii")).toBe("%PDF-");
    });

    it("reuses the exact same toCustomerOutstandingExportTable shaping as the xlsx path — one shaping function, two rendering targets", async () => {
      const sharedTables = [{ sheetName: "Customer Outstanding", columns: [], rows: [] }];
      toCustomerOutstandingExportTableMock.mockReturnValue(sharedTables);
