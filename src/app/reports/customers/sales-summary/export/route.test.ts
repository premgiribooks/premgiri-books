...
const {
  ...
  toCustomerSalesSummaryExportTableMock,
  ...
  toCustomerSalesSummaryExportTableMock: vi.fn(),
...
vi.mock("@/modules/reports/customers/services/customer-report-service", () => ({
  customerReportService: { getCustomerSalesSummary: getCustomerSalesSummaryMock },
}));
vi.mock("@/lib/excel-export", () => ({
  exportToExcelBuffer: exportToExcelBufferMock,
}));
vi.mock("@/engines/reporting/customer-reports", () => ({
  toCustomerSalesSummaryExportTable: toCustomerSalesSummaryExportTableMock,
}));
...
function request(query: string): Request {
  return new Request(`http://localhost/reports/customers/sales-summary/export${query}`);
}

const VALID_QUERY = "?dateFrom=2027-04-01&dateTo=2027-04-30";

describe("GET /reports/customers/sales-summary/export", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    getCurrentCompanyUserMock.mockResolvedValue({ id: "user-1", companyId: "company-1", role: "Accountant" });
    assertPermissionMock.mockResolvedValue(undefined);
    getCustomerSalesSummaryMock.mockResolvedValue({
      rows: [],
      totals: { invoiceCount: 0, taxableAmount: 0, totalTax: 0, grandTotal: 0 },
    } as PartyWiseSalesReport);
    toCustomerSalesSummaryExportTableMock.mockReturnValue([
      { sheetName: "Customer Sales Summary", columns: [], rows: [] },
    ]);
  ...
  it("maps AuthorizationError (missing reports:export) to 403, before the service is ever called", async () => {
    ...
    expect(getCustomerSalesSummaryMock).not.toHaveBeenCalled();
  });

  it("maps a plain AppError to 400", async () => {
    getCustomerSalesSummaryMock.mockRejectedValue(new AppError("Select an active financial year."));

    const response = await GET(request(VALID_QUERY));

    expect(response.status).toBe(400);
    expect(await response.json()).toEqual({ error: "Select an active financial year." });
  });
  ...
  it("returns the workbook buffer with the correct headers on success", async () => {
    ...
    expect(response.headers.get("Content-Disposition")).toBe(
      'attachment; filename="Customer-Sales-Summary-2027-04-01_to_2027-04-30.xlsx"'
    );
  ...
  describe("format=pdf", () => {
    it("returns a %PDF--prefixed buffer with the correct headers on success", async () => {
      ...
      expect(response.headers.get("Content-Disposition")).toBe(
        'attachment; filename="Customer-Sales-Summary-2027-04-01_to_2027-04-30.pdf"'
      );
      const buffer = Buffer.from(await response.arrayBuffer());
      expect(buffer.subarray(0, 5).toString("ascii")).toBe("%PDF-");
    });

    it("reuses the exact same toCustomerSalesSummaryExportTable shaping as the xlsx path — one shaping function, two rendering targets", async () => {
      const sharedTables = [{ sheetName: "Customer Sales Summary", columns: [], rows: [] }];
      toCustomerSalesSummaryExportTableMock.mockReturnValue(sharedTables);
    ...
    it("applies the same filter validation as the xlsx path", async () => {
      const response = await GET(request("?dateFrom=not-a-date&dateTo=2027-04-30&format=pdf"));
