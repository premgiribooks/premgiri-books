...
const {
  ...
  getSalaryRegisterMock,
  exportToExcelBufferMock,
  toSalaryRegisterExportTableMock,
  renderHtmlToPdfMock,
  buildReportHtmlMock,
  errorMock,
} = vi.hoisted(() => ({
  ...
  getSalaryRegisterMock: vi.fn(),
  exportToExcelBufferMock: vi.fn(),
  toSalaryRegisterExportTableMock: vi.fn(),
...
vi.mock("@/modules/reports/employees/services/employee-report-service", () => ({
  employeeReportService: { getSalaryRegister: getSalaryRegisterMock },
}));
vi.mock("@/lib/excel-export", () => ({
  exportToExcelBuffer: exportToExcelBufferMock,
}));
vi.mock("@/engines/reporting/employee-reports", () => ({
  toSalaryRegisterExportTable: toSalaryRegisterExportTableMock,
}));
...
function request(query: string): Request {
  return new Request(`http://localhost/reports/employees/salary-register/export${query}`);
}

const VALID_QUERY = "?employeeId=11111111-1111-4111-8111-111111111111";

describe("GET /reports/employees/salary-register/export", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    getCurrentCompanyUserMock.mockResolvedValue({ id: "user-1", companyId: "company-1", role: "Accountant" });
    assertPermissionMock.mockResolvedValue(undefined);
    getSalaryRegisterMock.mockResolvedValue({
      employeeId: "11111111-1111-4111-8111-111111111111",
      employeeName: "Asha Rao",
      rows: [],
    } as SalaryRegisterReport);
    toSalaryRegisterExportTableMock.mockReturnValue([
      { sheetName: "Salary Register", title: "Asha Rao", columns: [], rows: [] },
    ]);
  ...
  it("returns 400 for a missing/invalid employeeId — never reaching the service", async () => {
    const response = await GET(request("?employeeId=not-a-uuid"));
  ...
  it("maps AuthorizationError (missing reports:export) to 403, before the service is ever called", async () => {
    ...
    expect(getSalaryRegisterMock).not.toHaveBeenCalled();
  });

  it("maps a plain AppError (e.g. employee not found) to 400", async () => {
    getSalaryRegisterMock.mockRejectedValue(new AppError("Employee not found."));
  ...
  it("passes explicit financialYearId/dateFrom/dateTo filters through to the service", async () => {
    await GET(
      request(
        "?employeeId=11111111-1111-4111-8111-111111111111&financialYearId=22222222-2222-4222-8222-222222222222&dateFrom=2026-01-01&dateTo=2026-03-31"
      )
    );

    expect(getSalaryRegisterMock).toHaveBeenCalledWith({
      employeeId: "11111111-1111-4111-8111-111111111111",
  ...
  it("returns the workbook buffer with the correct headers, filename built from the resolved employeeName", async () => {
    const response = await GET(request(VALID_QUERY));
    ...
    expect(response.headers.get("Content-Disposition")).toBe('attachment; filename="Salary-Register-Asha_Rao.xlsx"');
  ...
  describe("format=pdf", () => {
    it("returns a %PDF--prefixed buffer with the correct headers, filename built from the resolved employeeName", async () => {
      const response = await GET(request(`${VALID_QUERY}&format=pdf`));

      expect(response.status).toBe(200);
      expect(response.headers.get("Content-Type")).toBe("application/pdf");
      expect(response.headers.get("Content-Disposition")).toBe('attachment; filename="Salary-Register-Asha_Rao.pdf"');
      const buffer = Buffer.from(await response.arrayBuffer());
      expect(buffer.subarray(0, 5).toString("ascii")).toBe("%PDF-");
    });

    it("reuses the exact same toSalaryRegisterExportTable shaping as the xlsx path — one shaping function, two rendering targets", async () => {
      const sharedTables = [{ sheetName: "Salary Register", title: "Asha Rao", columns: [], rows: [] }];
      toSalaryRegisterExportTableMock.mockReturnValue(sharedTables);
    ...
    it("applies the same filter validation as the xlsx path", async () => {
      const response = await GET(request("?employeeId=not-a-uuid&format=pdf"));
