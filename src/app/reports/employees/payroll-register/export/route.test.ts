...
describe("GET /reports/employees/payroll-register/export", () => {
  ...
  describe("format=pdf", () => {
    it("returns a %PDF--prefixed buffer with the correct headers on success", async () => {
      ...
      expect(response.headers.get("Content-Disposition")).toBe('attachment; filename="Payroll-Register.pdf"');
      const buffer = Buffer.from(await response.arrayBuffer());
      expect(buffer.subarray(0, 5).toString("ascii")).toBe("%PDF-");
    });

    it("reuses the exact same toPayrollRegisterExportTable shaping as the xlsx path — one shaping function, two rendering targets", async () => {
      const sharedTables = [{ sheetName: "Payroll Register", columns: [], rows: [] }];
      toPayrollRegisterExportTableMock.mockReturnValue(sharedTables);
    ...
    it("applies the same filter validation as the xlsx path", async () => {
      const response = await GET(request("?financialYearId=not-a-uuid&format=pdf"));
