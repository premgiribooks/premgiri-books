...
function downloadFilename(from: string, to: string): string {
  ...
  return `GST-Reports-${safeFrom}_to_${safeTo}.xlsx`;
}

/**
 * Delivers the GST Reports dashboard (trend + embedded HSN Summary) as a
 * downloadable `.xlsx`, following trial-balance/export/route.ts's own
 * reference wiring exactly. Re-checks its own `reports`/`export` permission
 * independently of the calling screen's Export button;
 ...
 * per gst-reports-service.ts's own doc comment).
 */
export async function GET(request: Request): Promise<NextResponse> {
  try {
    ...
    const buffer = await exportToExcelBuffer(toGstDashboardExportTable(report));
