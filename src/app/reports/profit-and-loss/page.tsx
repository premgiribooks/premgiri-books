...
import { ProfitAndLossStatement } from "@/modules/reports/components/profit-and-loss-statement";
...
export default async function ProfitAndLossPage({ searchParams }: ProfitAndLossPageProps) {
      ...
      <AppShell isAdmin={isAdmin}>
        <div className="flex flex-col gap-6 p-6">
          ...
          <div className="flex flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-border py-16 text-center">
            <p className="text-sm text-muted-foreground">Create a financial year before viewing Profit &amp; Loss.</p>
          </div>
        </div>
      </AppShell>
    );
  ...
  let report: ProfitAndLossReport | null = null;
  let errorMessage: string | null = null;
  try {
    report = await profitAndLossService.getProfitAndLoss({
    ...
    <AppShell isAdmin={isAdmin}>
      <div className="flex flex-col gap-6 p-6">
        ...
        {errorMessage ? (
          ...
          <ProfitAndLossStatement report={report!} />
        )}
      </div>
    </AppShell>
  );
