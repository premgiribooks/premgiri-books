...
vi.mock("@/modules/reports/services/balance-sheet-service", () => ({
  balanceSheetService: { getBalanceSheet: getBalanceSheetMock },
}));
...
function request(query: string): Request {
  return new Request(`http://localhost/reports/balance-sheet/export${query}`);
}
...
describe("GET /reports/balance-sheet/export", () => {
  beforeEach(() => {
    ...
    toBalanceSheetExportTableMock.mockReturnValue([
  ...
  it("maps a plain AppError (e.g. an invalid financial year) to 400", async () => {
    getBalanceSheetMock.mockRejectedValue(new AppError("Financial year not found."));
