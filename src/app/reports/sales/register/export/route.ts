...
function downloadFilename(dateFrom: string, dateTo: string, format: ExportFormat): string {
  ...
  return `Sales-Register-${safeFrom}_to_${safeTo}.${format}`;
}

/**
 * Delivers the Sales Register report as a downloadable `.xlsx` or `.pdf`,
 ...
 * of the format branch; `salesReportService.getSalesRegister` re-checks
 ...
 * apply exactly as it does for the on-screen report. Reuses the exact same
 * `toSalesRegisterExportTable(report)` shaping for both rendering targets —
