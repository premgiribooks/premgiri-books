...
const {
  ...
  getSalesRegisterMock,
  exportToExcelBufferMock,
  toSalesRegisterExportTableMock,
  ...
  getSalesRegisterMock: vi.fn(),
  exportToExcelBufferMock: vi.fn(),
  toSalesRegisterExportTableMock: vi.fn(),
...
vi.mock("@/modules/reports/sales/services/sales-report-service", () => ({
  salesReportService: { getSalesRegister: getSalesRegisterMock },
}));
vi.mock("@/lib/excel-export", () => ({
  exportToExcelBuffer: exportToExcelBufferMock,
}));
vi.mock("@/engines/reporting/sales-reports", () => ({
  toSalesRegisterExportTable: toSalesRegisterExportTableMock,
}));
...
function request(query: string): Request {
  return new Request(`http://localhost/reports/sales/register/export${query}`);
}

const VALID_QUERY = "?dateFrom=2027-01-01&dateTo=2027-03-31";
const EMPTY_REPORT: SalesRegisterReport = {
  rows: [],
  totals: { taxableAmount: 0, totalTax: 0, grandTotal: 0, amountPaid: 0 },
};

describe("GET /reports/sales/register/export", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    getCurrentCompanyUserMock.mockResolvedValue({ id: "user-1", companyId: "company-1", role: "Accountant" });
    assertPermissionMock.mockResolvedValue(undefined);
    getSalesRegisterMock.mockResolvedValue(EMPTY_REPORT);
    toSalesRegisterExportTableMock.mockReturnValue([{ sheetName: "Sales Register", columns: [], rows: [] }]);
  ...
  it("maps AuthorizationError (missing reports:export) to 403, before the service is ever called", async () => {
    ...
    expect(getSalesRegisterMock).not.toHaveBeenCalled();
  });

  it("maps a plain AppError to 400", async () => {
    getSalesRegisterMock.mockRejectedValue(new AppError("Something about the filters was invalid."));
  ...
  it("returns the workbook buffer with the correct headers on success", async () => {
    ...
    expect(response.headers.get("Content-Disposition")).toBe(
      'attachment; filename="Sales-Register-2027-01-01_to_2027-03-31.xlsx"'
    );
  ...
  it("succeeds with an omitted customerId/status — both optional, defaulting server-side", async () => {
    ...
    expect(response.status).toBe(200);
    expect(getSalesRegisterMock).toHaveBeenCalledWith(
      expect.objectContaining({ dateFrom: "2027-01-01", dateTo: "2027-03-31", status: "POSTED" })
    );
    expect(getSalesRegisterMock.mock.calls[0][0]).not.toHaveProperty("customerId");
  });
  ...
  describe("format=pdf", () => {
    it("returns a %PDF--prefixed buffer with the correct headers on success", async () => {
      ...
      expect(response.headers.get("Content-Disposition")).toBe(
        'attachment; filename="Sales-Register-2027-01-01_to_2027-03-31.pdf"'
      );
      const buffer = Buffer.from(await response.arrayBuffer());
      expect(buffer.subarray(0, 5).toString("ascii")).toBe("%PDF-");
    });

    it("reuses the exact same toSalesRegisterExportTable shaping as the xlsx path — one shaping function, two rendering targets", async () => {
      const sharedTables = [{ sheetName: "Sales Register", columns: [], rows: [] }];
      toSalesRegisterExportTableMock.mockReturnValue(sharedTables);
    ...
    it("renders A4 portrait per the reports paper-size convention", async () => {
      ...
      expect(renderHtmlToPdfOrHtmlMock).toHaveBeenCalledWith(expect.any(String), {
        format: "A4",
