...
const {
  ...
  toPartyWiseSalesExportTableMock,
  renderHtmlToPdfMock,
  buildReportHtmlMock,
  errorMock,
} = vi.hoisted(() => ({
  ...
  toPartyWiseSalesExportTableMock: vi.fn(),
...
vi.mock("@/modules/reports/sales/services/sales-report-service", () => ({
  salesReportService: { getPartyWiseSalesReport: getPartyWiseSalesReportMock },
}));
vi.mock("@/lib/excel-export", () => ({
  exportToExcelBuffer: exportToExcelBufferMock,
}));
vi.mock("@/engines/reporting/sales-reports", () => ({
  toPartyWiseSalesExportTable: toPartyWiseSalesExportTableMock,
}));
...
function request(query: string): Request {
  return new Request(`http://localhost/reports/sales/party-wise/export${query}`);
}

const VALID_QUERY = "?dateFrom=2027-01-01&dateTo=2027-03-31";
const EMPTY_REPORT: PartyWiseSalesReport = {
  rows: [],
  totals: { invoiceCount: 0, taxableAmount: 0, totalTax: 0, grandTotal: 0 },
};

describe("GET /reports/sales/party-wise/export", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    getCurrentCompanyUserMock.mockResolvedValue({ id: "user-1", companyId: "company-1", role: "Accountant" });
    assertPermissionMock.mockResolvedValue(undefined);
    getPartyWiseSalesReportMock.mockResolvedValue(EMPTY_REPORT);
    toPartyWiseSalesExportTableMock.mockReturnValue([{ sheetName: "Party-wise Sales", columns: [], rows: [] }]);
  ...
  it("maps AuthorizationError (missing reports:export) to 403, before the service is ever called", async () => {
    ...
    expect(getPartyWiseSalesReportMock).not.toHaveBeenCalled();
  });

  it("maps a plain AppError to 400", async () => {
    getPartyWiseSalesReportMock.mockRejectedValue(new AppError("Something about the filters was invalid."));
  ...
  it("returns the workbook buffer with the correct headers on success", async () => {
    ...
    expect(response.headers.get("Content-Disposition")).toBe(
      'attachment; filename="Party-wise-Sales-2027-01-01_to_2027-03-31.xlsx"'
    );
    ...
    expect(getPartyWiseSalesReportMock).toHaveBeenCalledWith({ dateFrom: "2027-01-01", dateTo: "2027-03-31" });
  });
  ...
  describe("format=pdf", () => {
    it("returns a %PDF--prefixed buffer with the correct headers on success", async () => {
      ...
      expect(response.headers.get("Content-Disposition")).toBe(
        'attachment; filename="Party-wise-Sales-2027-01-01_to_2027-03-31.pdf"'
      );
      const buffer = Buffer.from(await response.arrayBuffer());
      expect(buffer.subarray(0, 5).toString("ascii")).toBe("%PDF-");
    });

    it("reuses the exact same toPartyWiseSalesExportTable shaping as the xlsx path — one shaping function, two rendering targets", async () => {
      const sharedTables = [{ sheetName: "Party-wise Sales", columns: [], rows: [] }];
      toPartyWiseSalesExportTableMock.mockReturnValue(sharedTables);
