...
const {
  ...
  getPurchaseRegisterMock,
  exportToExcelBufferMock,
  toPurchaseRegisterExportTableMock,
  ...
  getPurchaseRegisterMock: vi.fn(),
  exportToExcelBufferMock: vi.fn(),
  toPurchaseRegisterExportTableMock: vi.fn(),
...
vi.mock("@/modules/reports/purchase/services/purchase-report-service", () => ({
  purchaseReportService: { getPurchaseRegister: getPurchaseRegisterMock },
}));
vi.mock("@/lib/excel-export", () => ({
  exportToExcelBuffer: exportToExcelBufferMock,
}));
vi.mock("@/engines/reporting/purchase-reports", () => ({
  toPurchaseRegisterExportTable: toPurchaseRegisterExportTableMock,
}));
...
function request(query: string): Request {
  return new Request(`http://localhost/reports/purchase/register/export${query}`);
}

const VALID_QUERY = "?dateFrom=2027-01-01&dateTo=2027-03-31&supplierId=11111111-1111-4111-8111-111111111111&status=POSTED";

describe("GET /reports/purchase/register/export", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    getCurrentCompanyUserMock.mockResolvedValue({ id: "user-1", companyId: "company-1", role: "Accountant" });
    assertPermissionMock.mockResolvedValue(undefined);
    getPurchaseRegisterMock.mockResolvedValue({
      rows: [],
      totals: { taxableAmount: 0, totalTax: 0, grandTotal: 0, amountPaid: 0 },
    } as PurchaseRegisterReport);
    toPurchaseRegisterExportTableMock.mockReturnValue([{ sheetName: "Purchase Register", columns: [], rows: [] }]);
  ...
  it("returns 400 for an invalid supplierId (not a uuid)", async () => {
    const response = await GET(request("?dateFrom=2027-01-01&dateTo=2027-03-31&supplierId=not-a-uuid"));
  ...
  it("maps AuthorizationError (missing reports:export) to 403, before the service is ever called", async () => {
    ...
    expect(getPurchaseRegisterMock).not.toHaveBeenCalled();
  });

  it("maps a plain AppError to 400", async () => {
    getPurchaseRegisterMock.mockRejectedValue(new AppError("Something went wrong reading the Purchase Register."));

    const response = await GET(request(VALID_QUERY));

    expect(response.status).toBe(400);
    expect(await response.json()).toEqual({ error: "Something went wrong reading the Purchase Register." });
  });
  ...
  it("returns the workbook buffer with the correct headers on success", async () => {
    ...
    expect(response.headers.get("Content-Disposition")).toBe(
      'attachment; filename="Purchase-Register-2027-01-01_to_2027-03-31.xlsx"'
    );
    ...
    expect(getPurchaseRegisterMock).toHaveBeenCalledWith({
      dateFrom: "2027-01-01",
      dateTo: "2027-03-31",
  ...
  it("succeeds when supplierId and status are both omitted from the query string, defaulting status to POSTED", async () => {
  ...
  describe("format=pdf", () => {
    it("returns a %PDF--prefixed buffer with the correct headers on success", async () => {
      ...
      expect(response.headers.get("Content-Disposition")).toBe(
        'attachment; filename="Purchase-Register-2027-01-01_to_2027-03-31.pdf"'
      );
      const buffer = Buffer.from(await response.arrayBuffer());
      expect(buffer.subarray(0, 5).toString("ascii")).toBe("%PDF-");
    });

    it("reuses the exact same toPurchaseRegisterExportTable shaping as the xlsx path — one shaping function, two rendering targets", async () => {
      const sharedTables = [{ sheetName: "Purchase Register", columns: [], rows: [] }];
      toPurchaseRegisterExportTableMock.mockReturnValue(sharedTables);
