...
function downloadFilename(dateFrom: string, dateTo: string): string {
  ...
  return `Purchase-Register-${safeDateFrom}_to_${safeDateTo}.xlsx`;
}

/**
 * Delivers the Purchase Register report as a downloadable `.xlsx`, following
 ...
 * screen's Export button; `purchaseReportService.getPurchaseRegister`
 ...
 * `purchaseRegisterFiltersSchema`'s own `.optional()`/`.default("POSTED")`
 * behave exactly as they do for a caller that never mentions them.
 */
export async function GET(request: Request): Promise<NextResponse> {
  try {
    ...
    const buffer = await exportToExcelBuffer(toPurchaseRegisterExportTable(report));
