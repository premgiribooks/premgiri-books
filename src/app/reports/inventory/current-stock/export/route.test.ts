...
vi.mock("@/modules/reports/inventory/services/inventory-report-service", () => ({
  inventoryReportService: { getCurrentStockReport: getCurrentStockReportMock },
}));
...
function request(query = ""): Request {
  return new Request(`http://localhost/reports/inventory/current-stock/export${query}`);
}

describe("GET /reports/inventory/current-stock/export", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("maps a plain AppError to 400", async () => {
    getCurrentStockReportMock.mockRejectedValue(new AppError("Something went wrong."));
  ...
  it("works with no filters at all", async () => {
    ...
    expect(getCurrentStockReportMock).toHaveBeenCalledWith({ includeZeroStock: false });
  });

  it("passes explicit productId/warehouseId/includeZeroStock filters through to the service", async () => {
    await GET(
      request(
        "?productId=11111111-1111-4111-8111-111111111111&warehouseId=22222222-2222-4222-8222-222222222222&includeZeroStock=true"
      )
    );

    expect(getCurrentStockReportMock).toHaveBeenCalledWith({
