...
const {
  ...
  getStockValuationReportMock,
  exportToExcelBufferMock,
  toStockValuationExportTableMock,
  renderHtmlToPdfMock,
  buildReportHtmlMock,
  errorMock,
} = vi.hoisted(() => ({
  ...
  getStockValuationReportMock: vi.fn(),
  exportToExcelBufferMock: vi.fn(),
  toStockValuationExportTableMock: vi.fn(),
...
vi.mock("@/modules/reports/inventory/services/inventory-report-service", () => ({
  inventoryReportService: { getStockValuationReport: getStockValuationReportMock },
}));
vi.mock("@/lib/excel-export", () => ({
  exportToExcelBuffer: exportToExcelBufferMock,
}));
vi.mock("@/engines/reporting/inventory-reports", () => ({
  toStockValuationExportTable: toStockValuationExportTableMock,
}));
...
function request(query = ""): Request {
  return new Request(`http://localhost/reports/inventory/valuation/export${query}`);
}

describe("GET /reports/inventory/valuation/export", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    getCurrentCompanyUserMock.mockResolvedValue({ id: "user-1", companyId: "company-1", role: "Accountant" });
    assertPermissionMock.mockResolvedValue(undefined);
    getStockValuationReportMock.mockResolvedValue({ rows: [], totalValue: 0 } as StockValuationReport);
    toStockValuationExportTableMock.mockReturnValue([{ sheetName: "Stock Valuation", columns: [], rows: [] }]);
  ...
  it("maps AuthorizationError (missing reports:export) to 403, before the service is ever called", async () => {
    ...
    expect(getStockValuationReportMock).not.toHaveBeenCalled();
  });

  it("maps a plain AppError to 400", async () => {
    getStockValuationReportMock.mockRejectedValue(new AppError("Something went wrong."));
  ...
  it("works with no filters at all", async () => {
    ...
    expect(getStockValuationReportMock).toHaveBeenCalledWith({});
  });
  ...
  it("passes an explicit warehouseId filter through to the service", async () => {
    ...
    expect(getStockValuationReportMock).toHaveBeenCalledWith({ warehouseId: "22222222-2222-4222-8222-222222222222" });
  });
  ...
  it("returns the workbook buffer with the correct headers on success", async () => {
    ...
    expect(response.headers.get("Content-Disposition")).toBe('attachment; filename="Stock-Valuation.xlsx"');
  ...
  describe("format=pdf", () => {
    it("returns a %PDF--prefixed buffer with the correct headers on success", async () => {
      ...
      expect(response.headers.get("Content-Disposition")).toBe('attachment; filename="Stock-Valuation.pdf"');
      const buffer = Buffer.from(await response.arrayBuffer());
      expect(buffer.subarray(0, 5).toString("ascii")).toBe("%PDF-");
    });

    it("reuses the exact same toStockValuationExportTable shaping as the xlsx path — one shaping function, two rendering targets", async () => {
      const sharedTables = [{ sheetName: "Stock Valuation", columns: [], rows: [] }];
      toStockValuationExportTableMock.mockReturnValue(sharedTables);
    ...
    it("applies the same filter validation as the xlsx path", async () => {
      const response = await GET(request("?warehouseId=not-a-uuid&format=pdf"));
