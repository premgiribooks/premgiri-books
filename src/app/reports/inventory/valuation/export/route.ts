...
function downloadFilename(format: ExportFormat): string {
  return `Stock-Valuation.${format}`;
}

/**
 * Delivers the Stock Valuation report as a downloadable `.xlsx` or `.pdf`,
 ...
 * calling screen's Export button; `inventoryReportService.getStockValuationReport`
 ...
 * `toStockValuationExportTable(report)` shaping for both rendering targets.
 */
export async function GET(request: Request): Promise<NextResponse> {
  try {
    ...
    const tables = toStockValuationExportTable(report);
